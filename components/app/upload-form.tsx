  ...
  return (
    <Card className="space-y-5 p-5 sm:p-6">
      ...
      <form action={formAction} className="space-y-4">
        ...
        <label
          className={`flex min-h-56 cursor-pointer flex-col items-center justify-center rounded-[24px] border border-dashed px-5 text-center transition ${
            isDragging
              ? "border-[var(--accent)] bg-[var(--accent-soft)]"
              : "border-[var(--line-strong)] bg-[var(--background-strong)]/60 hover:border-[var(--accent)] hover:bg-white"
          }`}
          onDragOver={(event) => {
        ...
        >
          <div className="mb-4 rounded-2xl bg-white p-4 shadow-[var(--shadow-soft)]">
            <UploadCloud className="h-6 w-6 text-[var(--accent)]" />
          </div>
          <p className="text-base font-semibold">{labels.dropzoneTitle}</p>
          <p className="mt-2 max-w-xs text-sm leading-6 text-[var(--muted)]">{labels.dropzoneText}</p>
          {selectedFile ? (
