...
import { getCasesWithActionNeeded, getProfile, getRecentDocuments, getTaskReminderBuckets, getUsageSummaryForCurrentUser } from "@/lib/queries";
...
export default async function AppHomePage() {
  const [profile, documents, reminderBuckets, usage, cases] = await Promise.all([
    getProfile(),
    getRecentDocuments(),
  ...
  return (
    <div className="space-y-10">
      <section className="space-y-3 pt-3">
        <p className="text-sm text-[var(--muted)]">{copy.home.welcome}</p>
        <h1 className="text-3xl font-semibold tracking-[-0.03em]">{profile?.full_name ?? copy.home.titleFallback}</h1>
        <p className="max-w-2xl text-sm leading-6 text-[var(--muted)]">{copy.home.intro}</p>
      </section>
          ...
          <section className="space-y-5 pt-2">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-[var(--muted)]">{reminderCopy.reminderIntro}</p>
              </div>
          ...
          <section className="space-y-5">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-[var(--muted)]">Hier siehst du deine offenen Bürokratie-Fälle als Verlauf.</p>
              </div>
