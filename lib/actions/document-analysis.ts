...
import { getAnalyzedDocumentName } from "@/lib/document-name";
...
export async function analyzeDocumentAction(
  ...
  try {
    ...
    const payload = {
      ...
      contract_parties: analysis.contract_parties,
      subject: analysis.subject,
      summary_simple: analysis.summary_simple,
      contract_summary_simple: analysis.contract_summary_simple,
      ...
      contract_action_points: analysis.contract_action_points,
      contract_duration: analysis.contract_duration,
      contract_notice_period: analysis.contract_notice_period,
      contract_recurring_costs: analysis.contract_recurring_costs,
      contract_auto_renewal: analysis.contract_auto_renewal,
      contract_clarification_points: analysis.contract_clarification_points,
      contract_flagged_clauses: analysis.contract_flagged_clauses,
      contract_flagged_points: analysis.contract_flagged_points,
      contract_possible_disadvantages: analysis.contract_possible_disadvantages,
      contract_pre_signing_checklist: analysis.contract_pre_signing_checklist,
      contract_watch_out_for: analysis.contract_watch_out_for,
      contract_watch_out_points: analysis.contract_watch_out_points,
      contract_unclear_points: analysis.contract_unclear_points,
      contract_risk_level_overview: analysis.contract_risk_level_overview,
    ...
    const nextDocumentName = getAnalyzedDocumentName({
      currentName: document.original_filename,
    ...
    if (nextDocumentName !== document.original_filename) {
      await supabase.from("documents").update({ original_filename: nextDocumentName }).eq("id", documentId).eq("user_id", user.id);
    }
