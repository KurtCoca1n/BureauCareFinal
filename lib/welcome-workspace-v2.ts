...
import type { CaseRecord, DocumentAnalysisRecord, DocumentRecord, DraftReplyRecord, TaskRecord } from "@/lib/types";
...
export type WelcomeWorkspaceCopy = {
  ...
  markTaskDoneLabel: string;
  savingTaskLabel: string;
};

const englishCopy: WelcomeWorkspaceCopy = {
  ...
  taskDoneLabel: "Done",
  markTaskDoneLabel: "Mark as done",
  savingTaskLabel: "Saving..."
};
...
const copyMap: Record<SupportedLanguage, WelcomeWorkspaceCopy> = {
  de: {
    ...
    markTaskDoneLabel: "Als erledigt markieren",
    savingTaskLabel: "Wird gespeichert..."
  },
  en: englishCopy,
  tr: englishCopy,
  uk: englishCopy,
  ...
  zh: {
    ...
    taskDoneLabel: "已完成",
    markTaskDoneLabel: "标记为已完成",
    savingTaskLabel: "正在保存..."
  }
};

export function getWelcomeWorkspaceCopy(locale: SupportedLanguage) {
  return copyMap[locale] ?? englishCopy;
}
...
export function buildWelcomeCaseTitle(stepKey: WelcomeStepKey, locale: SupportedLanguage) {
  ...
  const titles: Record<WelcomeStepKey, LocalizedText> = {
    ...
    health_insurance: { de: "Krankenversicherung", en: "Health insurance", zh: "健康保险" },
...
function getLocalizedText(text: LocalizedText | undefined, locale: SupportedLanguage) {
